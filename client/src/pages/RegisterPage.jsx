...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!termsAccepted) {
      setError('You must accept the Terms and Conditions and acknowledge the Privacy Policy to register.');
      return;
    }
      ...
      >
        <div style={{ marginBottom: '1.5rem', textAlign: 'center' }}>
          <h2 style={{ fontSize: '1.35rem', fontWeight: 700, color: 'var(--text-primary)' }}>
            Register New Vault Account
          </h2>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '4px' }}>
            Initialize your encrypted profile & local credentials
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
          ...
          {/* Mandatory Form Consent Checkbox */}
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.6rem', marginTop: '0.2rem' }}>
            ...
            />
            <label
              htmlFor="register-terms-consent"
            ...
            >
              ...
              <Link to="/privacy" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent-blue)', textDecoration: 'underline', fontWeight: 600 }}>
                Privacy Policy
              </Link>. I consent to the local processing of my document records.
            </label>
          </div>
          ...
          >
            <span>{loading ? 'Creating Vault...' : 'Create Secure Vault Account'}</span>
            <ArrowRight size={16} />
          </button>
        </form>
        ...
        <div style={{ textAlign: 'center', marginTop: '1.5rem', paddingTop: '1.25rem', borderTop: '1px solid var(--border-light)' }}>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
            ...
            <Link to="/login" style={{ color: 'var(--brand-primary-accessible)', fontWeight: 600 }}>
              Sign in to Vault instead
            </Link>
          </p>
        </div>
      </div>
